...
Usage: prepare.sh [-h] <issue_number>
...
Writes:
  issue.json              Issue metadata (number, title, body, current labels, repo description).
...
Expects:
  ...
  BLIZZ_BODY_LENGTH_LIMIT      (optional) Max body length before auto-triage. Default: 8000.
EOF
)"
...
ARG_ISSUE_NUMBER=""
BODY_LENGTH_LIMIT="${BLIZZ_BODY_LENGTH_LIMIT:-8000}"
...
: <<'DOC'
  Fetches the repo description from the GitHub API.
DOC
function fetch_repo_description() {
  gh api "repos/${GITHUB_REPOSITORY}" --jq '.description // "No description provided."'
}

: <<'DOC'
  Extracts the fields the triage prompt needs from the raw API response:
  number, title, body (truncated to 4K chars), current label names, and
  the repo name + description for project context.
DOC
function write_issue() {
  local raw_issue="$1"
  local repo_desc

  repo_desc=$(fetch_repo_description)

  echo "$raw_issue" \
    | jq --arg repo "$GITHUB_REPOSITORY" --arg desc "$repo_desc" '{
        repository: $repo,
