...
USAGE="$(cat <<EOF
Finds open issues not yet assessed by the Blizz triage workflow.
...
: <<'DOC'
  Searches for open issues that have not yet been processed by the Blizz
  workflow (no blizz-* labels). Results are sorted oldest-first so the
...
function fetch_untriaged_issues() {
  gh search issues \
    ...
    -- -label:blizz-rejected -label:blizz-candidate -label:blizz-greenlit \
    | jq -c '[.[].number]'
}
