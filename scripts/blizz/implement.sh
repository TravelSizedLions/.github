...
USAGE="$(cat <<EOF
Assigns the Copilot coding agent to all open issues tagged blizz-greenlit.

Usage: implement.sh [-h]

Copilot creates a branch and opens a PR for each assigned issue. Intended
to be triggered after a human reviews blizz-candidate issues and re-tags
the approved ones.
...
: <<'DOC'
  Fetches open issues tagged blizz-greenlit, capped at GREENLIT_SOFT_CAP.
DOC
function fetch_greenlit_issues() {
  gh issue list \
    --repo "$GITHUB_REPOSITORY" \
    --state open \
    --label "blizz-greenlit" \
    --json number \
